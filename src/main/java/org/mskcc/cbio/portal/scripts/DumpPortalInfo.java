...
public class DumpPortalInfo extends ConsoleRunnable {
    ...
    private static void writeJsonFile(
            ...
            try {
                OBJECT_MAPPER.writeValue(outputFile, objectList);
    ...
    public void run() {
        try {
            ...
            try {
                ...
                writeJsonFile(
                    DaoPortalGeneset.getAllGenesets(MAX_PAGE_SIZE, MIN_PAGE_NUMBER),
                    nameJsonFile(outputDir, API_GENESETS));
                writeJsonFile(
                    Arrays.asList(DaoPortalGeneset.getGenesetVersion()),
                    nameJsonFile(outputDir, API_GENESET_VERSION));
                writeJsonFile(
                    DaoPortalGenePanel.getAllGenePanels(MAX_PAGE_SIZE, MIN_PAGE_NUMBER),
