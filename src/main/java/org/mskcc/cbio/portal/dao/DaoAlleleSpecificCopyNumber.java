...
public final class DaoAlleleSpecificCopyNumber {
    ...
    public static int addAlleleSpecificCopyNumber(AlleleSpecificCopyNumber ascn) throws DaoException {
        if (!SQLiteBulkLoader.isBulkLoad()) {
            throw new DaoException("You have to turn on SQLiteBulkLoader in order to insert allele specific copy numbers");
        } else {
            int result = 1;
            SQLiteBulkLoader.getSQLiteBulkLoader("allele_specific_copy_number").insertRecord(
