...
function execute_sql_statement_via_clickhouse_client() {
    ...
    if [ -e "$output_filepath" ] && ! rm -f "$output_filepath" ; then
        echo "Error : could not overwrite existing output file $output_filepath when executing clickhouse statment $statement" >&2
    fi
    (
...
function execute_sql_statement_from_file_via_clickhouse_client() {
    ...
    local output_filepath=$2
    if [ -e "$output_filepath" ] && ! rm -f "$output_filepath" ; then
        echo "Error : could not overwrite existing output file $output_filepath when executing clickhouse statments from file $statement_filepath" >&2
    fi
    (
